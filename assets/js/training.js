...
render() {
    ...
    updateBank() {
        ...
        document.getElementById('ex-bank').innerHTML = this.data.exBank.map(x=>`<option value="${x}">`).join('');
    },
    ...
    delWeek(id) {
        ...
        this.pushHistory();
        // Знаходимо індекс за ID, це надійніше
   ...
   renderStats() {
        ...
        // Ми показуємо ВСІ тижні, щоб можна було порівняти, але сортуємо їх
        // Додаємо realIndex кожному тижню, щоб знати, звідки брати дані
        const allWeeks = this.data.weeks.map((w, idx) => ({ ...w, realIndex: idx }));

        // Якщо список пустий або кількість змінилась - перемальовуємо options
        // Або якщо це перший запуск
        if (sel.innerHTML === "" || sel.options.length !== allWeeks.length) {
            sel.innerHTML = allWeeks.map(w => {
                // Додаємо позначку про програму
            ...
            // Автовибір останнього тижня при першому відкритті
            if (allWeeks.length > 0) {
                sel.value = allWeeks[allWeeks.length - 1].realIndex;
            }
        }

        const wIdx = sel.value; // Це realIndex
        ...
        // --- Далі логіка підрахунку без змін ---
        const stats = {}; 
