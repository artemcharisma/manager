...
const PhotoDB = {
    ...
    add(week, file) {
        return new Promise((r) => {
            ...
            reader.onload = () => { 
                ...
                tx.objectStore("photos").add({ week: week, data: reader.result });
                r(); 
            };
    ...
    del(id) {
        return new Promise((r) => { 
            if(!this.db) { r(); return; }
            const tx = this.db.transaction(["photos"], "readwrite");
            tx.objectStore("photos").delete(id).onsuccess = () => r(); 
        });
    },
...
const App = {
    ...
    photoKeys: new Set(),
    history: [],
    state: { view: 'protocol', phaseId: 1, week: 1, editing: false, tempPill: null },
    chartInstance: null,
    dayBuffer: null,
    ...
    async init() {
        ...
        // Load via Utils
        const saved = Utils.load('gold_protocol', null);
        if(saved) {
            this.data = saved;
            // Defaults check
    ...
    save() { 
        Utils.save('gold_protocol', this.data); 
    },
    ...
    unlockPrivacy() {
        ...
        if (pwd === truePass) grantAccess(false);
        else if (pwd === fakePass) grantAccess(true);
        else {
    ...
    enableFakeMode() {
        ...
        c.innerHTML = `
            <div style="padding:20px; text-align:center; color:#888;">
                <h2>VITAMIN TRACKER</h2><p>Daily Wellness Plan</p>
            </div>
            <div class="days-grid">
    ...
    getRealDate(week, dayIndex) {
        ...
        return monday.toLocaleDateString('uk-UA', { day: '2-digit', month: '2-digit' });
    },
    ...
    pushHistory() {
        ...
        const undo = document.getElementById('undoFloat');
        if (undo && this.state.editing) undo.classList.add('visible');
    },
    undo() {
        if(this.history.length === 0) return;
        this.data = JSON.parse(this.history.pop());
        if(this.history.length === 0) document.getElementById('undoFloat').classList.remove('visible');
    ...
    async renderView() {
        ...
        const pct = Math.min(100, (this.state.week / maxW) * 100);
        document.getElementById('progBar').style.width = pct + '%';
        document.getElementById('progText').innerText = `Week ${this.state.week}/${maxW}`;
    ...
    async renderProtocol(c) {
        ...
        let statsHtml = Object.entries(stats).map(([k,v]) => {
            let color = 'yellow';
            ...
            grid += `<div class="day-card" style="${isToday ? 'border-color:var(--primary); box-shadow:0 0 10px rgba(212,175,55,0.1)' : ''}">
                <div class="day-header">
                    <div style="display:flex; flex-direction:column; line-height:1.2">
                        <span>${dayNames[i]}</span><span style="font-size:0.65rem; color:#666; font-weight:400">${realDate}</span>
                    </div>
                    ${copyBtn}
                </div>
                ${content}
    ...
    renderAnalytics(c) {
                ...
                this.data.schedule[w].forEach(day => day.forEach(pill => {
                    ...
                    if (match) {
                        ...
                        if (!isNaN(val) && unit.includes('mg') && ['c-blue', 'c-green', 'c-red', 'c-pink', 'c-yellow'].includes(pill.color)) {
                            if(name.includes('test') || name.includes('sust') || name.includes('enan')) weekTest += val;
                            else weekOther += val;
                        }
                    }
                }));
            }
            dataTest.push(weekTest); dataStack.push(weekOther);
        ...
        gradTest.addColorStop(0, 'rgba(212, 175, 55, 0.1)'); gradTest.addColorStop(1, 'rgba(212, 175, 55, 0.9)'); 
        const gradStack = ctx.createLinearGradient(0, 400, 0, 0);
        gradStack.addColorStop(0, 'rgba(139, 92, 246, 0.1)'); gradStack.addColorStop(1, 'rgba(139, 92, 246, 0.9)'); 
        ...
        this.chartInstance = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [
                    { label: 'Вага (kg)', data: dataWeight, type: 'line', borderColor: '#ffffff', backgroundColor: '#ffffff', borderWidth: 2, yAxisID: 'y1', pointRadius: 3, tension: 0.4 },
                    { label: 'Stack (mg)', data: dataStack, backgroundColor: gradStack, yAxisID: 'y', stack: 'total', borderRadius: 4 },
                    { label: 'Test Base (mg)', data: dataTest, backgroundColor: gradTest, yAxisID: 'y', stack: 'total', borderRadius: 4 }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                scales: {
                    x: { stacked: true, grid: { display: false }, ticks: { color: '#555' } },
                    y: { stacked: true, position: 'left', grid: { color: '#222', borderDash: [5, 5] }, title: { display: true, text: 'LOAD (mg)' } },
                    y1: { display: true, position: 'right', grid: { display: false }, title: { display: true, text: 'KG' } }
                }
            }
        });
    },

    // --- OTHER RENDER METHODS (Analysis, Pharmacy, BodyMap) ---
    renderAnalysis(c) {
            ...
            <div class="med-card">
                <div class="med-header">
                    <div style="flex-grow:1">
                        <div class="med-title" contenteditable="${this.state.editing}" onblur="App.data.analysis[${i}].title=this.innerText; App.save()">${block.title}</div>
                        <div class="med-timing" contenteditable="${this.state.editing}" onblur="App.data.analysis[${i}].timing=this.innerText; App.save()">${block.timing}</div>
                    </div>
                ...
                <div class="med-list">
                    ${block.checks.map((chk, j) => `<div class="check-row"><span class="check-icon">●</span><span class="check-name" contenteditable="${this.state.editing}" onblur="App.data.analysis[${i}].checks[${j}]=this.innerText; App.save()">${chk}</span></div>`).join('')}
                </div>
        ...
        html += `</div>`;
        if (this.state.editing) html += `<button class="btn-new-section" onclick="App.pushHistory(); App.data.analysis.push({title:'НОВИЙ ЕТАП', timing:'Тиждень ?', checks:['Показник']}); App.save(); App.renderView()">+ СТВОРИТИ ЕТАП</button>`;
        c.innerHTML = html;
    },
    ...
    renderPharm(c) {
        ...
        this.data.pharmacy.forEach((cat, i) => {
            ...
            <div class="category-block">
                <div class="category-header ${cat.style}"><span>${cat.title}</span></div>
                <div class="med-list">
                    ${cat.items.map((item, j) => `
                        <div class="med-item">
                            <div class="med-row-top">
                                <span class="med-name" contenteditable="${this.state.editing}" onblur="App.data.pharmacy[${i}].items[${j}].n=this.innerText; App.save()">${item.n}</span>
                                <div style="display:flex;align-items:center;gap:8px;">
                                    <span class="med-dose" contenteditable="${this.state.editing}" onblur="App.data.pharmacy[${i}].items[${j}].d=this.innerText; App.save()">${item.d}</span>
                                    ${this.state.editing ? `<span style="color:#ef4444;cursor:pointer;font-size:0.8rem" onclick="App.delMed(${i},${j})">✕</span>` : ''}
                                </div>
                            </div>
                            <div class="med-desc" contenteditable="${this.state.editing}" onblur="App.data.pharmacy[${i}].items[${j}].i=this.innerText; App.save()">${item.i}</div>
                        </div>`).join('')}
                </div>
    ...
    openBodyMap() { document.getElementById('bodyMapModal').style.display='flex'; this.renderBodyMap(); },
    renderBodyMap() {
        const muscles = [
    ...
    setDose(val) { document.getElementById('pillDose').value = val; },
    updateSuggestions() {
        ...
        const dl = document.getElementById('med-suggestions'); dl.innerHTML = '';
        Array.from(medSet).sort().forEach(m => { const opt = document.createElement('option'); opt.value = m; dl.appendChild(opt); });
        const tagContainer = document.getElementById('tagPresets'); tagContainer.innerHTML = '';
        Array.from(tagSet).sort().forEach(t => {
            if(t.length > 0) {
                const chip = document.createElement('div'); chip.className = 'tag-chip'; chip.innerText = t;
    ...
    confirmAddPill() {
        ...
        this.save(); this.closeModal(); this.renderView();
    },
    closeModal() { document.querySelectorAll('.modal').forEach(e=>e.style.display='none'); },
    updatePill(w,d,i,k,v) { this.pushHistory(); this.data.schedule[w][d][i][k]=v; this.save(); this.renderView(); },
    saveNote(w,t) { this.pushHistory(); this.data.notes[w]=t; this.save(); },
    saveVital(w,d,k,v) { this.pushHistory(); const key=`${w}-${d}`; if(!this.data.vitals[key]) this.data.vitals[key]={bp:"",hr:"",w:""}; this.data.vitals[key][k]=v; this.save(); },
    copyDay(w, d) {
    ...
    // UI Switchers
    setView(v, btn) { this.state.view = v; document.querySelectorAll('.nav-tab').forEach(e=>e.classList.remove('active')); btn.classList.add('active'); document.getElementById('phaseNav').style.display = v==='protocol'?'flex':'none'; this.renderView(); },
    setPhase(id) { this.state.phaseId = id; const ph = this.data.phases.find(p=>p.id===id); if(ph) { this.state.week = ph.weeks[0]; this.renderNav(); this.renderView(); } },
    setWeek(w) { this.state.week = w; this.renderView(); },
    
    // CRUD Logic
    delPillItem(w,d,i) { this.pushHistory(); this.data.schedule[w][d].splice(i,1); this.save(); this.renderView(); },
    addMed(catIdx) { let n=prompt("Назва:"); if(n) { this.pushHistory(); this.data.pharmacy[catIdx].items.push({n:n,d:"-",i:"-"}); this.save(); this.renderView(); } },
    delMed(c,i) { if(confirm("Видалити?")) { this.pushHistory(); this.data.pharmacy[c].items.splice(i,1); this.save(); this.renderView(); } },
    importData(inp) { const r=new FileReader(); r.onload=e=>{ try { this.pushHistory(); this.data=JSON.parse(e.target.result); this.save(); location.reload(); } catch(err) { alert("❌ Error"); } }; r.readAsText(inp.files[0]); },
    ...
    async addPhaseWeek(pId) { 
        ...
        for(let w = maxW; w > lastWeek; w--) { 
            ...
            for(let d=0; d<7; d++) if(this.data.vitals[`${w}-${d}`]) { this.data.vitals[`${w+1}-${d}`] = this.data.vitals[`${w}-${d}`]; delete this.data.vitals[`${w}-${d}`]; } 
        } 
        this.data.schedule[lastWeek + 1] = copyOfLastWeek;
        await PhotoDB.shiftWeeks(1); 
        phase.weeks.push(lastWeek + 1); 
        for(let i = pIdx + 1; i < this.data.phases.length; i++) this.data.phases[i].weeks = this.data.phases[i].weeks.map(w => w + 1);
        this.save(); this.refreshPhotos(); this.renderNav(); this.renderView(); 
    },
    async prependPhaseWeek(pId) {
        this.pushHistory();
        const pIdx = this.data.phases.findIndex(p => p.id === pId);
        if (pIdx !== 0) return alert("Тільки для першої фази.");
        const phase = this.data.phases[0];
        const maxW = Math.max(...Object.keys(this.data.schedule).map(Number));
        for(let w = maxW; w >= 1; w--) {
            this.data.schedule[w+1] = this.data.schedule[w];
            this.data.notes[w+1] = this.data.notes[w];
            for(let d=0; d<7; d++) if(this.data.vitals[`${w}-${d}`]) this.data.vitals[`${w+1}-${d}`] = this.data.vitals[`${w}-${d}`];
        }
        ...
        const d = new Date(this.data.startDate); d.setDate(d.getDate() - 7); this.data.startDate = d.toISOString().split('T')[0];
        this.save(); this.refreshPhotos(); this.renderNav(); this.renderView();
    },
    async removePhaseWeek(pId) { 
        ...
        for(let w = lastWeek; w < maxW; w++) { 
            ...
            for(let d=0; d<7; d++) if(this.data.vitals[`${w+1}-${d}`]) this.data.vitals[`${w}-${d}`] = this.data.vitals[`${w+1}-${d}`]; 
        } 
    ...
    deletePhase(pId) { 
        ...
        this.save(); this.renderNav(); this.setPhase(this.data.phases[0]?.id || 1); 
    },
    
    // Photos & Compare
    async refreshPhotos() { this.photoKeys = await PhotoDB.keys(); },
    async uploadPhoto(inp) { 
        ...
        await this.refreshPhotos(); this.renderView(); 
    },
    async deletePhoto(id) { if(confirm("Видалити фото?")) { this.pushHistory(); await PhotoDB.del(id); await this.refreshPhotos(); this.renderView(); } },
    ...
    async openCompareModal() {
        ...
        selL.innerHTML = createOpts(); selR.innerHTML = createOpts();
        selL.value = 1; selR.value = this.state.week;
        await this.loadCompareImage('L', 1); await this.loadCompareImage('R', this.state.week);
    },
    async loadCompareImage(side, week) {
        ...
        if(photos && photos.length > 0) box.innerHTML = `<img src="${photos[0].data}" style="width:100%; height:100%; object-fit:contain;">`;
        else box.innerHTML = `<div style="text-align:center; opacity:0.3"><div style="font-size:2rem">🚫</div><small>No Photo</small></div>`;
    },
    ...
    toggleEdit() { 
        ...
        else if(this.history.length > 0) document.getElementById('undoFloat').classList.add('visible');
